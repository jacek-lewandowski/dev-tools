...
bash "$REPO_ROOT/bin/ai/ai-sandbox-migrate" >"$tmp/m" 2>&1 || { echo FAILED; cat "$tmp/m"; }
...
bash "$REPO_ROOT/bin/ai/ai-sandbox-gc" </dev/null >"$tmp/gc" 2>&1 || true
...
bash "$REPO_ROOT/bin/ai/ai-sandbox-extensions" >"$tmp2/ext" 2>&1 || true
...
bash "$REPO_ROOT/bin/ai/create-ai-sandbox.sh" --display=none --no-start "$proj2" >>"$tmp2/o" 2>&1 || true
